...
時間制約の厳しいwebアプリケーションをjavascriptで作ったらそれなりに動いてしまいブラウザに可能性を感じたので記事に書きました。
この記事ではアプリの詳細は省いて技術的な事を中心に書きます。
もし技術ではなく、アプリ自体に興味を持たれたらこちらからどうぞ。
...
運転中に使われる想定のため、常時ブラウザが開き続けている必要があります。
...
このアプリケーションでほしい値はスマホを軸にした加速度の情報ではなく、車を軸にした加速度の情報です。
そのため「スマホの軸」を「車の軸」にあわせて回転する必要があります。

詳細は以下をご覧ください。
https://techblog.yahoo.co.jp/entry/2020120930052940/

なので、このアプリケーションで本当に必要な値は、スマホの加速度センサーで取得した値からノイズを除去するために平均化を行い、複雑な回転処理をおこなった結果の値となります。
...
このあたりは、実際にアプリケーションを現実世界で使いながら調整していく必要がります。

⑤回転処理をかけた値を音で出力
さきほどの「ユーザー操作を無視して音を出力させる」の都合で音声出力ロジックはsetInterval内で動いています。
なので回転処理をかけた値が確定した直後に鳴らす事はできません。
単純にsetIntervalの呼び出し間隔を速くしてしまうとアプリケーションが重くなってしまうので、こちらも②と同様に現実世界に合わせて調整していく必要があります。
...
メンテナンス性という点では、ios, android, webブラウザを意識して作ったので今後の保守は難しくなるので、それらを吸収できるフレームワークがあれば保守もしやすくなると思います。
