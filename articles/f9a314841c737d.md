...
この記事では、無料の地図データからこだわりの道を抽出する方法について解説します。
これは、地理空間情報の初心者の私がこだわりの道を探した時の方法です。
私自身何から触ればいいのか分からなかったので、同じ境遇の人の参考になれば幸いです。
...
OSMには膨大なデータが含まれているため、一気にすべての事を行うとすると大変です。
...
エッジは特定区間の道のような物です。
![エッジの一覧](/images/f9a314841c737d/3.png)

## 特徴を評価して絞り込む

さきほどのエッジ使ってさらに絞り込みを行います。
エッジは複数の点（緯度、経度）の集まりからできています。
![](/images/f9a314841c737d/4.png)

この点を使ってエッジを評価します。
評価値の上位数件が`こだわりの道`となりなります。
![](/images/f9a314841c737d/5.png)
![](/images/f9a314841c737d/6.png)

# 手を動かして実装する

こだわりの道を「らくして運転できる道」とし、抽出できるまでをゴールとします。
...
情報量が多すぎると、考える事が多くなってしまいます。
なので「楽して運転できる道」が含まれる3~5kmの範囲の地図データを使って調査を行います。

地図データの取得には[OSMnx](https://osmnx.readthedocs.io/en/stable/)というライブラリを使いました。
このライブラリはOSMのデータ取得、解析、視覚化をするためのライブラリです。

OSMの道には[道路種別](https://wiki.openstreetmap.org/wiki/JA:Key:highway)と[車線の数](https://wiki.openstreetmap.org/wiki/JA:Key:lanes)の情報を含んでいます。
OSMnxは条件を指定してOSMの地図データを取得できるので「`一般道`」と「`車線が２車線以上の道`」を抽出条件に含み、ここで確定させます。
![](/images/f9a314841c737d/7.png)

「`一般道`」と「`車線が２車線以上の道`」はこんな感じに取得できます。
...
直前で取得した地図データには、必要最低限の情報しか含めませんでした。
なので、すべての道を取得し、その道とエッジを比較して分岐数を計算します。
...
「分岐数が少ない道」と「直線的な道」の評価値との積を求めれば「らくして運転できる道」の評価値がとれます。
...
![](/images/f9a314841c737d/12.png)
これで、抽出と確認が行えるようになりました。

## 未知の道を抽出する

ここまでは、小さい範囲で評価関数を作成して抽出を行いました。
次は、範囲を広げて`未知の「楽して運転できる道」`を抽出します。
