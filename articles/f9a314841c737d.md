...
これは、初心者の私が手を動かした時の事を元にしています。
はじめてオープンデータに触れた時に何から触れば良いのかわからなかったので、同じ境遇の人の参考になれば幸いです。
...
「楽して運転できる道」が含まれる5kmの範囲の地図データを取得します。
広範囲で取得してしまうと、複雑でわかりにくくなるので範囲は狭めました。
...
OSMnxはOSMデータを取得する時に条件を指定できるので「`一般道`」と「`２車線以上の道`」を抽出条件に含ませて、ここで確定させます。
